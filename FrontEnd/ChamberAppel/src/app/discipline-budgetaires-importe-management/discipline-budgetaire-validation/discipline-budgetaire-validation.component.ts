...
@Component({
  selector: 'app-discipline-budgetaire-validation',
  templateUrl: './discipline-budgetaire-validation.component.html',
...
export class DisciplineBudgetaireValidationComponent implements OnInit {
  ...
  Analyse() {
    ImportService.postApiImportAnalyse(EnumFileType.DISCIPLINE_BUDGETAIRE).then(
  ...
  GetAllValideData() {
    ...
    ImportService.postApiImportGetAllDisciplineBudgetaireValide(
      this.pagination
    )
  ...
  GetAllErrorsData() {
    ...
    ImportService.postApiImportGetAllDisciplineBudgetaireErrors(
      this.pagination
    )
      .then((res: DisciplineBudgetaireTempDatatableResponse) => {
        this.Data = res.data ?? [];
